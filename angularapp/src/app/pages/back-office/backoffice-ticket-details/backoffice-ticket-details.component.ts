...
@Component({
  selector: 'app-backoffice-ticket-details',
  templateUrl: './backoffice-ticket-details.component.html',
  styleUrls: ['./backoffice-ticket-details.component.css']
})
export class BackofficeTicketDetailsComponent {
  title = 'customEditor';
  ...
  constructor(private confirmationService: ConfirmationService, private userService: UserService, private settingService: SettingsService, private logger: LoggerService, private router: Router, private cdr: ChangeDetectorRef, private ticketService: TicketService, private messageService: MessageService) { }
  async ngOnInit() {

    this.user = JSON.parse(localStorage.getItem("user") || "{}") as User;
    this.ticket = JSON.parse(localStorage.getItem("BackOfficeTicketDetails") || "{}") as Ticket;
  ...
  takeoverTicket() {
    this.ticketService.backOfficeAcceptTicket(this.ticket.id, this.user.user_Id).then(
      (res: any) => {
        ...
        this.ticket.assingedToBackOfficeID = this.user.user_Id;
        localStorage.setItem("BackOfficeTicketDetails", JSON.stringify(this.ticket));
        this.getComments();
        this.router.navigateByUrl("main/backoffice/tickets/details");
  ...
  resolveTicket() {
    this.ticketService.resolveTicket(this.ticket).then(
      (res: any) => {
        this.messageService.add({ severity: 'success', summary: 'Done ', detail: 'Ticket Resolved', life: 3000 });
        this.ticket.statusID = 4;
        this.ticket.updateByUser = this.user.user_Id;
        this.ticket.departmentReply = "Your issue has been resolved";
        localStorage.setItem("BackOfficeTicketDetails", JSON.stringify(this.ticket));
        this.getComments();
        this.router.navigateByUrl("main/backoffice/tickets/details");
  ...
  async createCommentAndClose() {
    this.confirmationService.confirm({
      message: 'Are you sure you want to add this reply as a customer offical reply for your department?\n This Reply will be sent to the customer directly',
      header: 'Confirm Department Reply',
      ...
      accept: () => {
        ...
        if (textWithoutTags === "" || textWithoutTags === "undefined") {
          ...
          this.comment.ticketID = this.ticket.id;
          this.comment.body = textWithoutTags;
          this.ticketService.addComment(this.comment).then(
            async (res: any) => {
              await this.ticketService.getlastCommentId().subscribe(
                (res: any) => {
                  this.comment.commentID = res;
                  ...
                  this.ticket.departmentReply = textWithoutTags;
                  await this.ticketService.resolveTicket(this.ticket).then(
                (res: any) => {
                  this.messageService.add({ severity: 'success', summary: 'Done ', detail: 'Ticket Resolved', life: 3000 });
                  this.ticket.statusID = 4;
                  this.ticket.departmentReply = textWithoutTags;
  ...
  assignToOtherAgent() {
    this.userService.getBackOfficeUsers().pipe(
      map((res: any) => {
        ...
        return {
          agents, // Include the original list of agents
          agentNameToAgentMap, // Include the mapping
        };
      })
    ).subscribe((data: any) => { // Specify the data type
      // Assign the agents list
      this.agents = data.agents;

      // Assign the mapping to the component property
  ...
  reOpenTicket() {
    this.ticketService.reopenInProgressTicket(this.ticket).then(
      (res: any) => {
